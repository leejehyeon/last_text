...
function update_form(id){
	var form = "form_" + id;
	var update_form = "update_form_" + id;
...
function reply_checkagain(id){
	var delete_reply_form = "delete_reply_board_" + id;
	var x = document.getElementById(delete_reply_form);
	if (confirm('정말로 삭제하시겠습니까?')) {
		x.submit();
	} else {
...
function change_subject_id(subject_id){
	 document.getElementById('subject_id').value = subject_id.value;
	 var url = "/index.php/question_and_answer/questioning_and_answering/"+subject_id.value;
     window.location.href = url;
}
...
function showtutee(subject){
	 ...
	 var month = document.getElementById('month').value;
	 var day = document.getElementById('day').value;
	 var user_subject = document.getElementById('user_subject1').value;
	 var user_divide = document.getElementById('user_divide1').value;
	 var form_url = "/index.php/lesson/get_user_by_divide/"+year+"/"+month+"/"+day+"?subject_id="+user_subject+"&user_divide="+user_divide;
	  if (subject=="") {
...
function register_form(attendance){
	...
	if($(":checkbox[name='chklist']:checked").length==0){
		...
		document.getElementById("check_length").value = $(":checkbox[name='chklist']:checked").length
		$(":checkbox[name='chklist']:checked").each(function(pi,po){
    	chklisk += ","+po.value;
 		});
 		if(chklisk!="")chklisk = chklisk.substring(1);
 			document.getElementById("user_id").value = chklisk;
 			document.getElementById("attendance").value = attendance;
 			alert(document.getElementById("check_length").value);
 			alert(document.getElementById("user_id").value);
 			alert(document.getElementById("attendance").value);
 			insert_attendance.submit();
	}else{}
}
...
function day_set(){
  ...
  var lastDay = date.getDate();
  makeOption('day',1,lastDay,1);
  makeOption2(1,lastYear,1);
  makeOption3(1,lastMonth,1);
}
...
function day_set2(){
  var set = document.day_form2;
  var date = new Date(set.year2.value,set.month2.value,0);
  var lastDay = date.getDate();
  makeOption2('day',1,lastDay,1);
}

//보강신청 년, 월, 일 select box의 option list 생성
function makeOption2(list,first,last,init) {
    var count = 0;
    var set = document.day_form2;
...
function changer_parent_licolor(obj){
	var li = obj.parentNode;
	var a = li.getElementsByTagName('a');
	a[0].style.color = "#e0e730";
	}
function changer_parent_licolor_out(obj){
	var li = obj.parentNode;
	var a = li.getElementsByTagName('a');
...
function Modify_reply(id){
	var form = "form_" + id;
	var update_form = "update_form_" + id;
	var textarea = document.getElementById('reply_textarea');
	if(document.getElementById(form).style.display != 'none'){
		document.getElementById(form).style.display = 'none'
		document.getElementById(update_form).style.display = '';
