...
class Ci_board extends CI_Model {
	...
	public function get_class_list_title3() {
		$this -> db -> select('board_id,subject,reg_date');
		$this -> db -> order_by('board_id', 'desc');
		$list = $this -> db -> get_where('ci_board', array('board_type' => 'class_notice'), 3, 0) -> result();
		return $list;
	}
	...
	public function get_journal_board_all($table = 'ci_board', $type = '', $offset = '', $limit = '') {
		...
		public function get_journal_list_title5() {
		$this -> db -> select('board_id,user_name,subject,reg_date');
		$this -> db -> order_by('board_id', 'desc');
		$list = $this -> db -> get_where('journal_board') -> result();
		return $list;
	}
		...
		public function load_journal_descript($jounal_data){
			$load = array('board_id' => $journal_data['board_id']);
			return $this -> db -> get_where('journal_board', array('board_id' => $load['board_id'])) -> row_array();
		}*/
